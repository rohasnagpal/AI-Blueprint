...
class ArbitrationPrepTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        workspace = cls.client.post("/api/v2/workspaces", json={"name": "Arbitration Workspace", "slug": "arb"})
        assert workspace.status_code == 201, workspace.text
        cls.workspace_id = workspace.json()["id"]
        matter = cls.client.post(f"/api/v2/workspaces/{cls.workspace_id}/matters", json={"name": "Arbitration Matter"})
        ...
        cls.indexed_doc_id = cls._insert_document(cls.matter_id, "pleading.txt", "indexed", "On 12 Jan 2026 Alpha Corp filed a claim for USD 100,000 damages. Jane Smith sent notice of breach. Procedural Order No. 1 sets a filing deadline of 2026-07-15. Confidential settlement communication may be privileged.")
    ...
    def test_arbitration_prep_run_persists_outputs_and_endpoints(self) -> None:
        response = self.client.post(
            f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs",
            json={
                "title": "Hearing Prep",
        ...
        result = self.client.get(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs/{queued['run_id']}")
        self.assertEqual(result.status_code, 200, result.text)
        body = result.json()
        self.assertEqual(body["title"], "Hearing Prep")
        ...
        endpoints = ["issues", "chronology", "evidence", "witnesses", "procedural-tasks", "audit-package"]
        for endpoint in endpoints:
            item = self.client.get(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs/{queued['run_id']}/{endpoint}")
            self.assertEqual(item.status_code, 200, f"{endpoint}: {item.text}")

        with SessionLocal() as db:
            self.assertIsNotNone(db.get(ArbitrationPrepRun, queued["run_id"]))
            self.assertEqual(db.query(ArbitrationPrepOutput).filter(ArbitrationPrepOutput.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationIssue).filter(ArbitrationIssue.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationEvidenceItem).filter(ArbitrationEvidenceItem.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationAgentStepOutput).filter(ArbitrationAgentStepOutput.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationChronologyEvent).filter(ArbitrationChronologyEvent.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationWitness).filter(ArbitrationWitness.run_id == queued["run_id"]).count(), 1)
            self.assertGreaterEqual(db.query(ArbitrationProceduralTask).filter(ArbitrationProceduralTask.run_id == queued["run_id"]).count(), 1)

    def test_validation_rejects_bad_sources_and_access(self) -> None:
        no_docs = self.client.post(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs", json={"matter_id": self.matter_id, "document_ids": []})
        self.assertEqual(no_docs.status_code, 400, no_docs.text)
        outside_matter = self.client.post(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs", json={"matter_id": self.matter_id, "document_ids": [self.other_doc_id]})
        self.assertEqual(outside_matter.status_code, 400, outside_matter.text)
        unindexed = self.client.post(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs", json={"matter_id": self.matter_id, "document_ids": [self.unindexed_doc_id]})
        ...
        with SessionLocal() as db:
            ...
            db.add(Workspace(id=other_workspace_id, name="Other Workspace", slug="other-arb", created_by_user_id=other_user_id))
        ...
        with TestClient(app) as outsider:
            ...
            denied = outsider.get(f"/api/v2/workspaces/{self.workspace_id}/arbitration-prep/runs")
    ...
    def test_each_tool_returns_expected_structure_and_verifies_anchors(self) -> None:
        source_bundle = [
            {
                ...
                "filename": "bundle.txt",
                "chunk_index": 0,
                "content": "On 12 Jan 2026 Jane Smith sent notice. Claimant seeks USD 100,000 damages. Confidential settlement email. Filing deadline 2026-07-15.",
                "page": 1,
                "start_offset": 0,
                "end_offset": 140,
            }
        ...
        result = run_arbitration_agent_tools(requests=requests, source_bundle=source_bundle, run_context={"party_role": "claimant", "hearing_dates": ["2026-07-15"]}, existing_outputs={"draft": {"source": {"excerpt": "On 12 Jan 2026 Jane Smith sent notice."}}})
        ...
        self.assertIn("required_sections", outputs["arbitration_audit_package_tool"])
        verifier = run_arbitration_agent_tools(
    ...
    def test_qc_flags_trigger_revision_and_clean_outputs_are_not_rerun(self) -> None:
        ...
        with patch("app.core.arbitration_agents.orchestrator.configured_llm_provider", return_value="mock"), patch("app.core.arbitration_agents.orchestrator.complete_with_configured_llm", side_effect=[json.dumps(item) for item in responses]) as mocked:
            result = run_agentic_arbitration_prep(sources=[], source_bundle=source_bundle, run_context={"forum_rules": "ICC", "seat": "Paris"}, settings=settings)
    ...
    def test_strict_planner_actions_drive_autonomous_loop(self) -> None:
        source_bundle = [{"document_id": "doc-1", "chunk_id": "chunk-1", "filename": "a.txt", "chunk_index": 0, "content": "Claim filed on 12 Jan 2026. Jane Smith sent notice.", "page": 1}]
        settings = {"chat_model": "mock-model"}
        responses = [
            {"next_actions": [
                {"type": "run_tool", "name": "targeted_document_retrieval", "reason": "Find claim evidence", "input": {"tool": "targeted_document_retrieval", "query": "claim notice"}},
                {"type": "run_agent", "name": "chronology_agent", "reason": "Build chronology", "input": {}},
                {"type": "finalize", "name": "finalize", "reason": "Bounded test complete", "input": {}},
            ], "evidence_gaps": ["Confirm forum rules"]},
            {"chronology": [{"date": "12 Jan 2026", "description": "Claim filed.", "source": {"excerpt": "Claim filed on 12 Jan 2026."}}]},
        ...
        self.assertEqual(loop["stop_reason"], "Bounded test complete")
