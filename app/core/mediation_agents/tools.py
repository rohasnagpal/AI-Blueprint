...
def build_default_tool_requests(run_context: dict[str, Any]) -> list[dict[str, Any]]:
    terms = [
        ...
        "claims defenses positions mediation settlement authority caucus damages",
    ]
    ...
    *,
    ...
    for request in normalized[:20]:
        ...
        if tool not in SUPPORTED_TOOLS:
            results.append({"tool": tool or "unknown", "status": "rejected", "error": "Unsupported mediation agent tool."})
            continue
...
def _run_tool(tool: str, request: dict[str, Any], source_bundle: list[dict[str, Any]], run_context: dict[str, Any], existing_outputs: dict[str, Any]) -> Any:
    ...
    if tool == "mediation_audit_package_tool":
        return _mediation_audit_package_tool(source_bundle, run_context, existing_outputs)
    return {}
    ...
    for source in source_bundle:
        ...
        for match in date_pattern.finditer(content):
            ...
            events.append({
                ...
                "relevance": "Potential mediation chronology event requiring lawyer review.",
...
def _claim_defense_mapper(source_bundle: list[dict[str, Any]]) -> list[dict[str, Any]]:
    terms = ["breach", "payment", "damages", "defense", "counterclaim", "settlement", "authority", "mediation"]
    claims = []
    for term in terms:
        ...
        if matches:
            claims.append({
                ...
                "defenses": [],
                "admissions": [],
                "missing_proof": ["Confirm position, support, authority, and mediation relevance with counsel."],
                "source": matches[0],
    ...
    return [
        {
            ...
            "prep_questions": [f"Confirm {name}'s role, authority, knowledge, and mediation talking points."],
    ...
    return [
        {
            ...
            "author": None,
            "recipient": None,
            "relevance": "Indexed matter source for mediation preparation.",
            "cited_issues": [],
...
def _procedural_deadline_tool(source_bundle: list[dict[str, Any]], run_context: dict[str, Any]) -> list[dict[str, Any]]:
    tasks = []
    keywords = ("deadline", "mediation", "statement", "brief", "attendance", "authority", "confidential", "submission", "calendar")
    for source in source_bundle:
        content = str(source.get("content") or "")
        if any(word in content.lower() for word in keywords):
            tasks.append({"task_type": "mediation_logistics", "description": content[:700], "due_date": _first_date(content), "compliance_risk": "Requires lawyer confirmation against mediation order, protocol, or provider rules.", "source": _source_anchor(source, content[:700]), "confidence_score": 0.68})
    for value in run_context.get("hearing_dates") or []:
        tasks.append({"task_type": "mediation_date", "description": f"User-provided mediation/deadline date: {value}", "due_date": value, "compliance_risk": "User-provided date; verify against mediation notice, protocol, or order.", "source": None, "confidence_score": 0.7})
    return tasks[:60]
...
def _discovery_gap_analyzer(source_bundle: list[dict[str, Any]]) -> list[dict[str, Any]]:
    findings = []
    keywords = ("information", "documents", "valuation", "expert", "insurance", "authority", "settlement", "mediation", "production")
    for source in source_bundle:
        content = str(source.get("content") or "")
        lower = content.lower()
    ...
    if not findings:
        findings.append({"item_type": "gap", "description": "No mediation information-exchange materials were detected in the indexed source set.", "status": "requires_review", "confidence_score": 0.45})
    return findings[:60]
...
def _trial_theme_builder(existing_outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "exhibit_groups": [],
        "caucus_topics": [],
        "caveat": "Mediation session plan outputs require lawyer review and do not predict outcomes.",
    }
...
def _mediation_audit_package_tool(source_bundle: list[dict[str, Any]], run_context: dict[str, Any], existing_outputs: dict[str, Any]) -> dict[str, Any]:
    return {
