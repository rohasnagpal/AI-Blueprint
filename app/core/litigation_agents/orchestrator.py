...
def _interim_qc(run_state: dict[str, Any], action: dict[str, Any], result: dict[str, Any] | None, source_bundle: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if action.get("type") in {"run_agent", "verify_anchors"}:
        verifier = run_litigation_agent_tools(requests=[{"tool": "evidence_anchor_verifier"}], source_bundle=source_bundle, run_context=run_state.get("matter_context", {}), existing_outputs=(result or {}).get("output") or result or {})
