...
from app.core.litigation_agents.agents import AGENT_SPECS, fallback_agent_output
from app.core.litigation_agents.tools import run_litigation_agent_tools
...
def run_agentic_litigation_prep(
    *,
    ...
    planner_data, planner_error = _run_agent(
        "litigation_planner_agent",
        "Plan the next bounded autonomous litigation preparation actions based on court, jurisdiction, venue, procedural stage, party role, documents, current working memory, open questions, tool results, and instructions.",
        ...
        model=model,
        max_tokens=1400,
    ) if provider else ({"strategy": "Run the default litigation preparation sequence.", "required_agents": [spec[0] for spec in AGENT_SPECS], "tool_requests": None, "evidence_gaps": [], "stop_conditions": []}, None)
    if planner_error:
        trace.append(_trace("litigation_planner_agent", "failed", planner_error, provider, model, None))
        outputs["litigation_planner_agent"] = {"strategy": "Run the default litigation preparation sequence.", "required_agents": []}
    else:
        outputs["litigation_planner_agent"] = planner_data
        trace.append(_trace("litigation_planner_agent", "completed", None, provider or "internal", model, None))
    _merge_planner_state(run_state, outputs.get("litigation_planner_agent", {}))

    action_queue = _planner_actions(outputs.get("litigation_planner_agent", {}), run_context)
    ...
    for step_number in range(1, MAX_AUTONOMOUS_STEPS + 1):
        if not action_queue:
            if provider and replan_count < MAX_REPLANS:
                ...
                if planner_error:
                    ...
                    break
                outputs[f"litigation_planner_agent_replan_{replan_count}"] = planner_data
                trace.append(_trace("litigation_planner_agent", "completed", None, provider, model, None))
        ...
        if error:
            ...
            _emit_progress(progress_callback, step_number, f"Litigation step failed: {step_name}", {"action": action, "error": error})
            continue
        if action_type == "run_tool":
            ...
            trace.append(_trace("agent_tool_controller", "completed", None, "internal", "litigation_agent_tools_v1", elapsed_ms))
    ...
    if not tool_results.get("tool_results"):
        started = time.perf_counter()
        tool_results = run_litigation_agent_tools(requests=None, source_bundle=source_bundle, run_context=run_context, existing_outputs=outputs)
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        outputs["agent_tool_controller"] = tool_results
        state["tool_results"] = tool_results
        trace.append(_trace("agent_tool_controller", "completed", None, "internal", "litigation_agent_tools_v1", elapsed_ms))
    ...
    agentic_review = {
        "planner_output": outputs.get("litigation_planner_agent", {}),
        "tool_requests": _tool_requests_from_steps(autonomous_steps) or outputs.get("litigation_planner_agent", {}).get("tool_requests") or [],
...
def _initial_run_state(run_context: dict[str, Any], sources: list[dict[str, Any]], source_bundle: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "objective": "Prepare an evidence-grounded litigation preparation package from indexed matter documents.",
...
def _planner_actions(planner_data: dict[str, Any], run_context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(tool_requests, list) or not tool_requests:
        tool_requests = [
            {"tool": "targeted_document_retrieval", "query": " ".join(str(item) for item in [run_context.get("party_role"), run_context.get("court"), run_context.get("jurisdiction"), run_context.get("procedural_stage"), "claims defenses discovery depositions motions trial deadlines damages"] if item)},
            ...
            {"tool": "litigation_audit_package_tool"},
        ]
    actions = [
...
def _execute_action(
    ...
    if action_type in {"run_tool", "verify_anchors"}:
        ...
        result = run_litigation_agent_tools(requests=[request], source_bundle=source_bundle, run_context=run_context, existing_outputs=outputs)
    ...
    if action_type == "run_agent":
        ...
        if not spec:
            return {}, f"Unsupported litigation specialist agent: {name}"
...
def _run_agent(agent_id: str, role: str, contract: str, payload: dict[str, Any], settings: dict[str, Any], *, model: str | None, max_tokens: int) -> tuple[dict[str, Any], str | None]:
    system = (
        f"You are {agent_id}, one specialist in a multi-agent litigation preparation system. "
...
def _deterministic_outputs(sources: list[dict[str, Any]], source_bundle: list[dict[str, Any]], run_context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "case_snapshot": {"court": run_context.get("court"), "jurisdiction": run_context.get("jurisdiction"), "venue": run_context.get("venue"), "procedural_stage": run_context.get("procedural_stage"), "requires_review": True},
        "claims_and_defenses": [{"claim_type": "unclassified", "title": "Claims and defenses require lawyer classification", "elements": [], "defenses": [], "admissions": [], "missing_proof": ["Confirm pleadings, elements, defenses, admissions, and denials."], "source": source, "confidence_score": 0.4}],
        ...
        "discovery_analysis": [{"item_type": "gap", "description": "Discovery posture requires lawyer review against pleadings, served discovery, responses, orders, and deadlines.", "status": "requires_review", "confidence_score": 0.4}],
        "witness_prep": [],
        "deposition_prep": [],
        "motion_strategy": {"motions": [], "caveat": "Motion strategy requires lawyer review and procedural verification."},
        "trial_prep": {"themes": [], "caveat": "Trial prep requires lawyer review and does not predict outcomes."},
        "argument_strategy": {"themes": [], "caveat": "No legal advice or outcome prediction. Strategy themes require lawyer review."},
        "cross_examination": [],
        "procedural_tasks": [],
        ...
        "client_or_team_summary": "Litigation preparation package generated from indexed matter documents. Human lawyer review is required before use or circulation.",
    }
...
def _warnings(run_context: dict[str, Any], qc: dict[str, Any]) -> list[str]:
    warnings = [
        "AI-assisted litigation preparation. Human legal review is required before use or circulation.",
...
def _emit_progress(callback: ProgressCallback | None, step_number: int, message: str, metadata: dict[str, Any]) -> None:
    if not callback:
        return
    ...
    step = str(action.get("name") or action.get("type") or f"step_{step_number}")
    try:
