...
def build_default_tool_requests(run_context: dict[str, Any]) -> list[dict[str, Any]]:
    terms = [
        ...
        "claim defense pleading discovery deposition motion trial damages",
    ]
...
def run_litigation_agent_tools(
    *,
    ...
    for request in normalized[:20]:
        ...
        if tool not in SUPPORTED_TOOLS:
            results.append({"tool": tool or "unknown", "status": "rejected", "error": "Unsupported litigation agent tool."})
            continue
...
def _run_tool(tool: str, request: dict[str, Any], source_bundle: list[dict[str, Any]], run_context: dict[str, Any], existing_outputs: dict[str, Any]) -> Any:
    ...
    if tool == "litigation_audit_package_tool":
        return _litigation_audit_package_tool(source_bundle, run_context, existing_outputs)
    return {}
    ...
    for source in source_bundle:
        ...
        for match in date_pattern.finditer(content):
            ...
            events.append({
                ...
                "relevance": "Potential litigation chronology event requiring lawyer review.",
...
def _claim_defense_mapper(source_bundle: list[dict[str, Any]]) -> list[dict[str, Any]]:
    terms = ["breach", "negligence", "fraud", "payment", "damages", "defense", "counterclaim", "affirmative defense"]
    claims = []
    for term in terms:
        ...
        if matches:
            claims.append({
                ...
                "defenses": [],
                "admissions": [],
                "missing_proof": ["Confirm elements, burden, and pleadings with counsel."],
                "source": matches[0],
    ...
    return [
        {
            ...
            "author": None,
            "recipient": None,
            "relevance": "Indexed matter source for litigation preparation.",
            "cited_issues": [],
...
def _discovery_gap_analyzer(source_bundle: list[dict[str, Any]]) -> list[dict[str, Any]]:
    findings = []
    keywords = ("interrogatory", "request for production", "rfp", "discovery", "objection", "deposition", "subpoena", "esi", "production")
    for source in source_bundle:
        content = str(source.get("content") or "")
        lower = content.lower()
    ...
    if not findings:
        findings.append({"item_type": "gap", "description": "No discovery materials were detected in the indexed source set.", "status": "requires_review", "confidence_score": 0.45})
    return findings[:60]
...
def _motion_argument_outline_tool(run_context: dict[str, Any], existing_outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"party_role": role, "motions": [{"title": "Evidence-backed motion issue", "points": [str(item.get("issue") or item.get("element") or "Issue") for item in matrix[:8]], "vulnerabilities": ["Verify procedural posture and admissible evidence."], "caveat": "Motion themes are preparation prompts, not legal advice or outcome predictions."}]}
...
def _trial_theme_builder(existing_outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "witness_order_considerations": [item.get("name") for item in witnesses[:8] if item.get("name")],
        "exhibit_groups": [],
        "objections": [],
        "caveat": "Trial prep outputs require lawyer review and do not predict outcomes.",
    }
...
def _litigation_audit_package_tool(source_bundle: list[dict[str, Any]], run_context: dict[str, Any], existing_outputs: dict[str, Any]) -> dict[str, Any]:
    return {
