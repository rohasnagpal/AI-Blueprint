...
def run_agentic_arbitration_prep(
    *,
    ...
    planner_data, planner_error = _run_agent(
        "arbitration_planner_agent",
        "Plan the next bounded autonomous arbitration preparation actions based on forum, stage, party role, documents, current working memory, open questions, tool results, and instructions.",
        ...
        model=model,
        max_tokens=1400,
    ) if provider else ({"strategy": "Run the default arbitration preparation sequence.", "required_agents": [spec[0] for spec in AGENT_SPECS], "tool_requests": None, "evidence_gaps": [], "stop_conditions": []}, None)
    if planner_error:
        trace.append(_trace("arbitration_planner_agent", "failed", planner_error, provider, model, None))
        outputs["arbitration_planner_agent"] = {"strategy": "Run the default arbitration preparation sequence.", "required_agents": []}
    else:
        outputs["arbitration_planner_agent"] = planner_data
        trace.append(_trace("arbitration_planner_agent", "completed", None, provider or "internal", model, None))
    _merge_planner_state(run_state, outputs.get("arbitration_planner_agent", {}))

    action_queue = _planner_actions(outputs.get("arbitration_planner_agent", {}), run_context)
    ...
    for step_number in range(1, MAX_AUTONOMOUS_STEPS + 1):
        if not action_queue:
            if provider and replan_count < MAX_REPLANS:
                ...
                if planner_error:
                    ...
                    break
                outputs[f"arbitration_planner_agent_replan_{replan_count}"] = planner_data
                trace.append(_trace("arbitration_planner_agent", "completed", None, provider, model, None))
        ...
        if action_type == "run_tool":
            ...
            trace.append(_trace("agent_tool_controller", "completed", None, "internal", "arbitration_agent_tools_v1", elapsed_ms))
    ...
    if not tool_results.get("tool_results"):
        started = time.perf_counter()
        tool_results = run_arbitration_agent_tools(requests=None, source_bundle=source_bundle, run_context=run_context, existing_outputs=outputs)
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        outputs["agent_tool_controller"] = tool_results
        state["tool_results"] = tool_results
        trace.append(_trace("agent_tool_controller", "completed", None, "internal", "arbitration_agent_tools_v1", elapsed_ms))
    ...
    agentic_review = {
        "planner_output": outputs.get("arbitration_planner_agent", {}),
        "tool_requests": _tool_requests_from_steps(autonomous_steps) or outputs.get("arbitration_planner_agent", {}).get("tool_requests") or [],
...
def _initial_run_state(run_context: dict[str, Any], sources: list[dict[str, Any]], source_bundle: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "objective": "Prepare an evidence-grounded arbitration preparation package from indexed matter documents.",
...
def _planner_actions(planner_data: dict[str, Any], run_context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(tool_requests, list) or not tool_requests:
        tool_requests = [
            {"tool": "targeted_document_retrieval", "query": " ".join(str(item) for item in [run_context.get("party_role"), run_context.get("forum_rules"), run_context.get("procedural_stage"), "claims evidence witnesses deadlines damages"] if item)},
...
def _execute_action(
    ...
    if action_type in {"run_tool", "verify_anchors"}:
        ...
        result = run_arbitration_agent_tools(requests=[request], source_bundle=source_bundle, run_context=run_context, existing_outputs=outputs)
    ...
    if action_type == "run_agent":
        ...
        if not spec:
            return {}, f"Unsupported arbitration specialist agent: {name}"
...
def _human_review_gates(run_state: dict[str, Any], flags: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not context.get("forum_rules") or context.get("forum_rules") == "other":
        gates.append({"gate": "missing_forum_rules", "reason": "Arbitration rules are absent or unspecified.", "requires_human_review": True})
    if not context.get("seat"):
        gates.append({"gate": "missing_seat", "reason": "Seat or jurisdiction is absent.", "requires_human_review": True})
    for flag in flags:
...
def _run_agent(agent_id: str, role: str, contract: str, payload: dict[str, Any], settings: dict[str, Any], *, model: str | None, max_tokens: int) -> tuple[dict[str, Any], str | None]:
    system = (
        f"You are {agent_id}, one specialist in a multi-agent arbitration preparation system. "
...
def _deterministic_outputs(sources: list[dict[str, Any]], source_bundle: list[dict[str, Any]], run_context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "case_snapshot": {"forum_rules": run_context.get("forum_rules"), "seat": run_context.get("seat"), "procedural_stage": run_context.get("procedural_stage"), "requires_review": True},
        ...
        "client_or_team_summary": "Arbitration preparation package generated from indexed matter documents. Human lawyer review is required before use or circulation.",
    }
...
def _warnings(run_context: dict[str, Any], qc: dict[str, Any]) -> list[str]:
    warnings = [
        "AI-assisted arbitration preparation. Human legal review is required before use or circulation.",
        "This output is not legal advice and does not predict outcomes.",
    ]
    if not run_context.get("forum_rules") or run_context.get("forum_rules") == "other":
        warnings.append("Arbitration rules are absent or unspecified; assumptions must be verified.")
    if not run_context.get("seat"):
        warnings.append("Seat or jurisdiction is absent; procedural assumptions must be verified.")
