...
def _format_output(output: ContractReviewOutput) -> dict:
    return {
        ...
        "extraction": _json_loads(output.extraction_json, {}),
        "risk_matrix": _json_loads(output.risk_matrix_json, []),
...
async def create_run(
    ...
    db.refresh(run)
    db.refresh(job)
    background_tasks.add_task(execute_contract_review, job.id, run.id)
