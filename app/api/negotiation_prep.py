...
from app.core.negotiation_agents.tools import SUPPORTED_TOOLS
...
from app.core.models import (
    NegotiationAgentStepOutput,
    NegotiationArgument,
    NegotiationChronologyEvent,
    NegotiationClaim,
    NegotiationDamagesItem,
    NegotiationDepositionTopic,
    NegotiationDiscoveryItem,
    NegotiationEvidenceItem,
    NegotiationIssue,
    NegotiationMotion,
    NegotiationPrepOutput,
    NegotiationPrepRun,
    NegotiationProceduralTask,
    NegotiationReviewDecision,
    NegotiationRiskItem,
    NegotiationWitness,
    BlueprintInstance,
    BlueprintMember,
...
router = APIRouter(prefix="/workspaces/{workspace_id}/negotiation-prep", tags=["negotiation-prep"])


class NegotiationPrepRunIn(BaseModel):
    ...
    negotiation_focus: str | None = Field(default=None, max_length=100)
    instructions: str | None = Field(default=None, max_length=12000)


class NegotiationDecisionIn(BaseModel):
...
def _source_chunks(db: Session, workspace_id: str, body: NegotiationPrepRunIn) -> list[tuple[KnowledgeChunk, KnowledgeDocument]]:
    ...
    if not chunks:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No indexed source chunks are available for negotiation preparation")
    return chunks
...
def _negotiation_blueprint(db: Session, workspace_id: str, matter: Matter, user: User) -> BlueprintInstance:
    plugin = db.get(Plugin, "negotiation_prep")
    if not plugin:
        plugin = Plugin(
            id="negotiation_prep",
        ...
        db.add(plugin)
        db.flush()
    name = "Standalone Negotiation Prep"
    blueprint = db.execute(
        select(BlueprintInstance).where(
            ...
            BlueprintInstance.plugin_id == "negotiation_prep",
    ...
    blueprint = BlueprintInstance(
        ...
        matter_id=matter.id,
        plugin_id="negotiation_prep",
        name=name,
        description="Hidden system blueprint used to persist standalone negotiation prep agent runs.",
        status="active",
...
def _format_run_row(run: NegotiationPrepRun) -> dict[str, Any]:
...
def _format_output(run: NegotiationPrepRun, output: NegotiationPrepOutput) -> dict[str, Any]:
    return {
        "id": run.id,
        "title": run.title,
        "status": run.status,
...
def _get_run_output(db: Session, workspace_id: str, run_id: str) -> tuple[NegotiationPrepRun, NegotiationPrepOutput]:
    run = db.execute(select(NegotiationPrepRun).where(NegotiationPrepRun.workspace_id == workspace_id, NegotiationPrepRun.id == run_id)).scalar_one_or_none()
    if not run:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Negotiation prep run not found")
    output = db.execute(select(NegotiationPrepOutput).where(NegotiationPrepOutput.run_id == run_id)).scalar_one_or_none()
    if not output:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Negotiation prep output not found")
    return run, output


def _execute_negotiation_prep(
    db: Session,
    *,
    workspace_id: str,
    body: NegotiationPrepRunIn,
    user: User,
    ...
    blueprint = _negotiation_blueprint(db, workspace_id, matter, user)
    if job:
        update_job_status(db, job=job, status="running", progress=10, message="Negotiation prep started")
    ...
    config = {
        ...
        "negotiation_focus": body.negotiation_focus,
        "instructions": body.instructions or "",
        "supported_tools": sorted(SUPPORTED_TOOLS),
    }
    if job:
        add_job_event(db, job=job, event_type="progress", message="Running negotiation agents", metadata={"progress": 45, "run_id": run_id})
    ...
    agentic = run_agentic_negotiation_prep(sources=sources, source_bundle=source_bundle, run_context=config, settings=settings, progress_callback=progress if job else None)
    provider = configured_llm_provider(settings)
    model = settings.get("chat_model") if provider else None
    if job:
        add_job_event(db, job=job, event_type="progress", message="Persisting negotiation prep outputs", metadata={"progress": 82, "run_id": run_id})
        job.progress = max(job.progress, 82)
        db.commit()
    _persist_negotiation_state(db, workspace_id=workspace_id, matter_id=matter.id, blueprint_id=blueprint.id, run_id=run_id, user=user, title=body.title or "Negotiation Prep", config=config, result=agentic, sources=sources, provider=provider, model=model)
    payload = _format_output(*_get_run_output(db, workspace_id, run_id))
    record_audit_event(
        db,
        action="negotiation_prep.run",
        resource_type="negotiation_prep",
        resource_id=run_id,
        user_id=user.id,
        workspace_id=workspace_id,
...
def _persist_negotiation_state(db: Session, *, workspace_id: str, matter_id: str, blueprint_id: str, run_id: str, user: User, title: str, config: dict[str, Any], result: dict[str, Any], sources: list[dict[str, Any]], provider: str | None, model: str | None) -> None:
    now = utcnow()
    run = NegotiationPrepRun(
        id=run_id,
        workspace_id=workspace_id,
        matter_id=matter_id,
        ...
        title=title,
        status="completed",
        status_detail="Agentic negotiation preparation completed.",
        config_snapshot_json=json.dumps(config, sort_keys=True),
        workflow_version="negotiation_prep_workflow_v2",
...
def _persist_step_outputs(db: Session, workspace_id: str, matter_id: str, run_id: str, config: dict[str, Any], result: dict[str, Any], provider: str | None, model: str | None) -> None:
    ...
    for step in result.get("agent_trace", []):
        ...
        db.add(NegotiationAgentStepOutput(id=str(uuid.uuid4()), workspace_id=workspace_id, matter_id=matter_id, run_id=run_id, step_name=step_name, step_version="agentic_negotiation_prep_v1", status=step.get("status") or "completed", input_json=json.dumps(config, sort_keys=True), output_json=json.dumps(outputs.get(step_name, {}), sort_keys=True), provider=step.get("provider") or provider, model=step.get("model") or model, error=step.get("error"), metadata_json=json.dumps({"duration_ms": step.get("duration_ms")}, sort_keys=True), started_at=utcnow(), completed_at=utcnow()))


def _run_negotiation_prep_job(job_id: str, workspace_id: str, user_id: str, body_data: dict[str, Any], run_id: str) -> None:
    with SessionLocal() as db:
        ...
        if not job or not user:
            return
        try:
            body = NegotiationPrepRunIn(**body_data)
            payload = _execute_negotiation_prep(db, workspace_id=workspace_id, body=body, user=user, run_id=run_id, job=job)
            metadata = json_loads(job.metadata_json, {})
            metadata.update({"run_id": run_id, "result_url": f"/api/v2/workspaces/{workspace_id}/negotiation-prep/runs/{run_id}"})
            job.metadata_json = json.dumps(metadata, sort_keys=True)
            update_job_status(db, job=job, status="completed", progress=100, message="Negotiation prep completed")
            add_job_event(db, job=job, event_type="result", message="Negotiation prep result ready", metadata={"run_id": run_id, "title": payload["title"]})
            db.commit()
        except Exception as exc:
            update_job_status(db, job=job, status="failed", progress=job.progress, message="Negotiation prep failed", error=str(exc))
            db.commit()
...
async def list_runs(workspace_id: str, page: int = Query(default=1, ge=1), page_size: int = Query(default=25, ge=1, le=100), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    rows = select(NegotiationPrepRun).where(NegotiationPrepRun.workspace_id == workspace_id).order_by(NegotiationPrepRun.created_at.desc())
    return page_query_response(db, rows, _format_run_row, page=page, page_size=page_size, scalars=True)


@router.post("/runs")
async def create_run(workspace_id: str, body: NegotiationPrepRunIn, background_tasks: BackgroundTasks, sync: bool = Query(default=False), user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    if sync:
        payload = _execute_negotiation_prep(db, workspace_id=workspace_id, body=body, user=user)
        db.commit()
        return payload
    _validate_matter(db, workspace_id, body.matter_id)
    _source_chunks(db, workspace_id, body)
    run_id = str(uuid.uuid4())
    job = create_job(db, workspace_id=workspace_id, created_by_user_id=user.id, job_type="negotiation_prep.run", metadata={"run_id": run_id, "title": body.title or "Negotiation Prep"}, message="Negotiation prep queued")
    record_audit_event(db, action="negotiation_prep.queue", resource_type="negotiation_prep", resource_id=run_id, user_id=user.id, workspace_id=workspace_id, metadata={"job_id": job.id, "document_count": len(set(body.document_ids)), "matter_id": body.matter_id})
    db.commit()
    db.refresh(job)
    background_tasks.add_task(run_background_job, job.id, _run_negotiation_prep_job, job.id, workspace_id, user.id, body.model_dump(), run_id)
...
async def delete_run(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.delete(run)
    record_audit_event(db, action="negotiation_prep.delete", resource_type="negotiation_prep", resource_id=run_id, user_id=user.id, workspace_id=workspace_id, metadata={"matter_id": matter_id})
    db.commit()
...
async def get_claims(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [_claim_payload(item) for item in db.execute(select(NegotiationClaim).where(NegotiationClaim.run_id == run_id).order_by(NegotiationClaim.created_at)).scalars().all()]
...
async def get_issues(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_issue_payload(item) for item in db.execute(select(NegotiationIssue).where(NegotiationIssue.run_id == run_id).order_by(NegotiationIssue.created_at)).scalars().all()]
...
async def get_chronology(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_row_payload(item, ["event_date", "description", "relevance", "anchor_text"]) for item in db.execute(select(NegotiationChronologyEvent).where(NegotiationChronologyEvent.run_id == run_id).order_by(NegotiationChronologyEvent.event_date, NegotiationChronologyEvent.created_at)).scalars().all()]
...
async def get_evidence(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_row_payload(item, ["issue_id", "evidence_type", "summary", "anchor_text"]) for item in db.execute(select(NegotiationEvidenceItem).where(NegotiationEvidenceItem.run_id == run_id).order_by(NegotiationEvidenceItem.created_at)).scalars().all()]
...
async def get_witnesses(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_witness_payload(item) for item in db.execute(select(NegotiationWitness).where(NegotiationWitness.run_id == run_id).order_by(NegotiationWitness.name)).scalars().all()]
...
async def get_depositions(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_deposition_payload(item) for item in db.execute(select(NegotiationDepositionTopic).where(NegotiationDepositionTopic.run_id == run_id).order_by(NegotiationDepositionTopic.created_at)).scalars().all()]
...
async def get_discovery(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_row_payload(item, ["item_type", "description", "status"]) for item in db.execute(select(NegotiationDiscoveryItem).where(NegotiationDiscoveryItem.run_id == run_id).order_by(NegotiationDiscoveryItem.created_at)).scalars().all()]
...
async def get_motions(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_motion_payload(item) for item in db.execute(select(NegotiationMotion).where(NegotiationMotion.run_id == run_id).order_by(NegotiationMotion.created_at)).scalars().all()]
...
async def get_procedural_tasks(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    _get_run_output(db, workspace_id, run_id)
    return [_row_payload(item, ["task_type", "description", "due_date", "compliance_risk"]) for item in db.execute(select(NegotiationProceduralTask).where(NegotiationProceduralTask.run_id == run_id).order_by(NegotiationProceduralTask.due_date, NegotiationProceduralTask.created_at)).scalars().all()]
...
async def get_audit_package(workspace_id: str, run_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    steps = db.execute(select(NegotiationAgentStepOutput).where(NegotiationAgentStepOutput.run_id == run_id).order_by(NegotiationAgentStepOutput.created_at)).scalars().all()
    decisions = db.execute(select(NegotiationReviewDecision).where(NegotiationReviewDecision.run_id == run_id).order_by(NegotiationReviewDecision.created_at)).scalars().all()
...
async def create_decision(workspace_id: str, run_id: str, body: NegotiationDecisionIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    run, _output = _get_run_output(db, workspace_id, run_id)
    decision = NegotiationReviewDecision(id=str(uuid.uuid4()), workspace_id=workspace_id, matter_id=run.matter_id, run_id=run_id, user_id=user.id, target_type=body.target_type, target_id=body.target_id, decision=body.decision, note=body.note, prior_status_json="{}", metadata_json=json.dumps(body.metadata, sort_keys=True))
    db.add(decision)
    record_audit_event(db, action="negotiation_prep.decision", resource_type="negotiation_prep", resource_id=run_id, user_id=user.id, workspace_id=workspace_id, metadata={"target_type": body.target_type, "target_id": body.target_id, "decision": body.decision})
    db.commit()
    db.refresh(decision)
...
async def list_templates(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    require_workspace_member(workspace_id, user, db)
    return [
        {"id": "standard-negotiation-prep", "name": "Standard Negotiation Prep", "description": "Issues, chronology, evidence, witnesses, procedure, damages, risks, and audit package."},
        {"id": "hearing-prep", "name": "Negotiation Prep", "description": "Emphasizes witness prep, cross-examination, chronology, and exhibits."},
...
def _issue_payload(item: NegotiationIssue) -> dict[str, Any]:
...
def _claim_payload(item: NegotiationClaim) -> dict[str, Any]:
...
def _witness_payload(item: NegotiationWitness) -> dict[str, Any]:
...
def _deposition_payload(item: NegotiationDepositionTopic) -> dict[str, Any]:
...
def _motion_payload(item: NegotiationMotion) -> dict[str, Any]:
...
def _decision_payload(item: NegotiationReviewDecision) -> dict[str, Any]:
